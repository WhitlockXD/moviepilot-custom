...
SPIDER_PARSER_CLASSES = {
    ...
    "PirateBaySpider": PirateBaySpider,
    "LeetxSpider": LeetxSpider,
    "BitSearchSpider": BitSearchSpider,
}
...
class IndexerModule(_ModuleBase):
    ...
    @staticmethod
    def get_public_indexers() -> List[dict]:
        """
        获取内置公共资源站点列表。

        这些站点为公开资源站，无需用户添加站点或认证即可搜索；
        域名可通过 SystemConfigKey.SearchPublicIndexers 关联配置覆盖默认镜像。
        :return: 公共索引器配置列表
        """
    ...
    def search_torrents(self, site: dict,
        ...
        try:
            ...
            elif site.get('parser') in ("PirateBaySpider", "LeetxSpider", "BitSearchSpider"):
                # 公共资源站：仅需关键字与页码，无需认证
                spider_cls = SPIDER_PARSER_CLASSES[site.get('parser')]
                error_flag, result = spider_cls(site).search(
                    keyword=search_word,
                    page=page
                )
            else:
    ...
    async def async_search_torrents(self, site: dict,
        ...
        try:
            if site.get('parser') == "TNodeSpider":
                ...
                # 公共资源站：仅需关键字与页码，无需认证
                spider_cls = SPIDER_PARSER_CLASSES[site.get('parser')]
                error_flag, result = await spider_cls(site).async_search(
                    keyword=search_word,
                    page=page
                )
            else:
