...
class SearchChain(ChainBase):
    ...
    @staticmethod
    def _public_indexer_enabled() -> bool:
        """
        内置公共索引器是否启用；未配置时默认启用，使无站点环境也能搜索公共资源。
        """
        value = SystemConfigOper().get(SystemConfigKey.SearchPublicIndexers)
        # 仅在显式关闭时禁用
        return value is not False

    @staticmethod
    def _has_english_indexer(indexer_sites: List[dict]) -> bool:
        """
        判断参与搜索的索引器中是否存在英文站点（用于决定是否需要中文转英文译名）。
        """
        return any((site or {}).get("language") == "en" for site in indexer_sites)

    def _resolve_indexer_sites(self, sites: Optional[List[int]]) -> List[dict]:
        """
        解析本次搜索的索引器列表：用户配置站点 + 内置公共站点。

        :param sites: 指定站点ID列表；为空表示搜索全部已启用站点
        :return: 实际参与搜索的索引器配置列表
        """
        if not sites:
        ...
        # 内置公共资源站：无需用户添加站点即可搜索
        if self._public_indexer_enabled():
            for indexer in self.run_module("get_public_indexers") or []:
                if not sites or indexer.get("id") in sites:
                    indexer_sites.append(indexer)
        return indexer_sites

    async def _async_resolve_indexer_sites(self, sites: Optional[List[int]]) -> List[dict]:
        """
        异步解析本次搜索的索引器列表：用户配置站点 + 内置公共站点。
        """
        if not sites:
            sites = SystemConfigOper().get(SystemConfigKey.IndexerSites) or []
        indexer_sites = []
        for indexer in await SitesHelper().async_get_indexers():
            if not sites or indexer.get("id") in sites:
                indexer_sites.append(indexer)
        if self._public_indexer_enabled():
            for indexer in await self.async_run_module("get_public_indexers") or []:
                if not sites or indexer.get("id") in sites:
                    indexer_sites.append(indexer)
        return indexer_sites

    def _resolve_english_keyword(self, keyword: Optional[str],
                                 mediainfo: Optional[MediaInfo] = None,
                                 mtype: MediaType = None) -> Optional[str]:
        """
        将中文搜索关键字转换为英文译名，供英文资源站搜索。

        优先使用已识别媒体的英文标题；缺失时通过 TMDB 按中文标题回查英文译名。
        :return: 可用于英文站点搜索的关键字；非中文关键字原样返回
        """
        if not keyword or not StringUtils.is_chinese(keyword):
            return keyword
        # 优先使用已识别媒体的英文标题
        if mediainfo:
            en_title = mediainfo.en_title or mediainfo.original_title
            if en_title and not StringUtils.is_chinese(en_title):
                return en_title
        # 通过 TMDB 按中文标题查询英文译名
        info = self.run_module("match_tmdbinfo", name=keyword, mtype=mtype)
        if info:
            en_title = info.get("en_title") or info.get("original_title")
            if en_title and not StringUtils.is_chinese(en_title):
                return en_title
        logger.warn(f"关键字 {keyword} 未能转换为英文译名，英文资源站将跳过")
        return keyword

    async def _async_resolve_english_keyword(self, keyword: Optional[str],
                                             mediainfo: Optional[MediaInfo] = None,
                                             mtype: MediaType = None) -> Optional[str]:
        """
        异步将中文搜索关键字转换为英文译名，供英文资源站搜索。
        """
        if not keyword or not StringUtils.is_chinese(keyword):
            return keyword
        if mediainfo:
            ...
            if en_title and not StringUtils.is_chinese(en_title):
                return en_title
        info = await self.async_run_module("async_match_tmdbinfo", name=keyword, mtype=mtype)
        if info:
            en_title = info.get("en_title") or info.get("original_title")
            if en_title and not StringUtils.is_chinese(en_title):
                return en_title
    ...
    @staticmethod
    def _site_search_keyword(site: dict,
                             keyword: Optional[str],
                             mediainfo: Optional[MediaInfo],
                             area: Optional[str],
                             english_keyword: Optional[str]) -> Optional[str]:
        """
        计算单个站点本次搜索应使用的关键字。

        IMDb ID 搜索区域直接用媒体 IMDb ID；英文站点用英文译名；其余用原始关键字。
        """
        if area == "imdbid" and mediainfo:
            return mediainfo.imdb_id
        if (site or {}).get("language") == "en" and english_keyword:
    ...
    def __search_all_sites(self, keyword: str,
        ...
        :reutrn: 资源列表
        """
        # 解析参与搜索的索引器：用户配置站点 + 内置公共站点
        indexer_sites = self._resolve_indexer_sites(sites)
        if not indexer_sites:
            logger.warn('未开启任何有效站点，无法搜索资源')
            return []
        # 存在英文站点时，将中文关键字转换为英文译名，避免英文站点因中文被跳过
        english_keyword = None
        if self._has_english_indexer(indexer_sites):
            english_keyword = self._resolve_english_keyword(
                keyword=keyword,
                mediainfo=mediainfo,
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            def submit_site_page(site: dict, page_index: int):
                ...
                search_keyword = self._site_search_keyword(
                    site=site, keyword=keyword, mediainfo=mediainfo,
                    area=area, english_keyword=english_keyword,
                )
    ...
    async def __async_search_all_sites(self, keyword: str,
        ...
        """
        # 解析参与搜索的索引器：用户配置站点 + 内置公共站点
        indexer_sites = await self._async_resolve_indexer_sites(sites)
        if not indexer_sites:
            logger.warn('未开启任何有效站点，无法搜索资源')
            return []
        # 存在英文站点时，将中文关键字转换为英文译名
        english_keyword = None
        if self._has_english_indexer(indexer_sites):
            english_keyword = await self._async_resolve_english_keyword(
                keyword=keyword,
                mediainfo=mediainfo,
                mtype=mediainfo.type if mediainfo else None,
            )
        ...
        async def search_site_page(site: dict, search_page: int) -> List[TorrentInfo]:
            ...
            async with semaphore:
                site_keyword = self._site_search_keyword(
                    site=site, keyword=keyword, mediainfo=mediainfo,
                    area=area, english_keyword=english_keyword,
                )
